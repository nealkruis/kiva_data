...
with open('../SS-Sens/solns.csv','rU') as f:
    ...
    for row in reader:
        ...
        if os.path.exists('../SS-Sens/'+dir_name+'/run.sh'):
            os.remove('../SS-Sens/'+dir_name+'/run.sh')
        if os.path.exists('../SS-Sens/'+dir_name+'/jrun.sh'):
            os.remove('../SS-Sens/'+dir_name+'/jrun.sh')
